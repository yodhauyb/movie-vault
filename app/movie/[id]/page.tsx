...
async function fetchTMDBDetails(id: string) {
  // Pehle TV Show (Web Series) check karenge kyunki zyadatar series hain
  let res = await fetch(`https://api.themoviedb.org/3/tv/${id}?api_key=${TMDB_API_KEY}&language=en-US`);
  if (res.ok) {
    const data = await res.json();
    return { ...data, media_type: 'tv' };
  }
  
  // Agar TV show nahi mila toh Movie check karenge
  res = await fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${TMDB_API_KEY}&language=en-US`);
  if (res.ok) {
    const data = await res.json();
...
export default async function MovieDetails({ params }: { params: Promise<{ id: string }> }) {
      ...
      <div className="min-h-screen bg-black text-white flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-neutral-400 text-lg">Loading content or Content not found...</p>
          </div>
        </div>
      </div>
    );
  ...
  // Faltu episodes ya 40 seasons wala glitch rokne ke liye clean filter
  if (details.media_type === 'tv' && rawLinks.length > 6) {
    rawLinks = rawLinks.filter((link, idx) => idx < 6 || link.toLowerCase().includes('season') || link.toLowerCase().includes('s0'));
    if (rawLinks.length === 0) rawLinks = linkData.split(" | ").slice(0, 5);
  }
